...
if (song === 'intro') {
  ...
  for (;;) {
    ...
    if (s.perf) lastScores = s.perf.scores.map((x) => [+x.q.toFixed(2), +x.timing.toFixed(2), x.count, x.feedback]).concat([['warmth', +s.perf.warmth.toFixed(2)]]);
  ...
  console.log('scores', JSON.stringify(lastScores));
